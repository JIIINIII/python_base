...
for i in range(len(mpgLst)):
    displ.append(float(mpgLst[i][2]))
for i in range(len(mpgLst)):
    hwy.append(float(mpgLst[i][8]))
dihwlst = list(zip(displ,hwy))

for i in range(len(dihwlst)):
    if dihwlst[i][0] <= 4:
        lower4.append(dihwlst[i][1])
    else:
        upper5.append(dihwlst[i][1])
